...
export default class UserTable extends React.Component {
  ...
  drop = (e: any) => {
    if (!e.target.innerHTML) {
      e.target.innerHTML = '<p>' + this.state.users[e.dataTransfer.getData('id') - 1].first_name +
        '<span id="' + 'remove_' + e.target.id.split('_')[1] + '_' + e.dataTransfer.getData('id') + '" onclick="this.remove(event)">X</span></p>'
  ...
  remove = (e: any) => {  
    let dropable_id = e.target.id.split('_')[1]
    let userList_id = e.target.id.split('_')[2]
  ...
  render() {
    ...
    for (var i = 0; i < 9; i++){
      let cell = []
      for (var j = 0; j < 5; j++){
        cell.push(<div className="user-cell" id={`dropable_${(i * 5 + j)}`} key={j} onDrop={this.drop} onDragOver={this.dragOver} onDragLeave={this.dragLeave}></div>)
      }
